...
RSpec.describe GameModule do
  ...
  describe '#author_choice' do
    it 'returns a new author' do
      ...
      allow(self).to receive(:fetch_author_data).and_return(['author1', 'author2'])
  ...
  describe '#game_list_display' do
    it 'displays game list' do
      allow(self).to receive(:fetch_game_data).and_return([{ "id" => 1, "author" => { "first_name" => "John", "last_name" => "Doe" }, "multiplayer" => false, "last_played_at" => "01-01-2023" }])
